...
import { FormControl, Grid, Container, Card, Stack, TextField, Box, InputLabel, Select, MenuItem } from '@mui/material';
...
export default function RequestEdit() {
  ...
  const handleStartDateChange = (date) => {
    ...
    if (dayPosition === 2) {
      ...
      additionalDays = 2; // Add Two extra days if the day is Monday (position 4)
  ...
  useEffect(() => {
    const token = localStorage.getItem('accessToken');
    axios
      .get('https://techstephub.focusrtech.com:3030/techstep/api/Timesheet/Service/mywfh/request', {
        headers: {
      ...
      .then((response) => {
        ...
        const statusFind = newArr[0].status;
        console.log('status', statusFind);
        console.log('This Is NewArr', newArr);
        setStartDate(newArr[0].fromdate);
        setEndDate(newArr[0].todate);
        setNumberOfDays(newArr[0].numberOfDays);
        setReason(newArr[0].reason);
        setPdoManager(newArr[0].projectmanager);
        setBackLogs(newArr[0].backlog);
        setRequestStatus(statusFind);
        setId(newArr[0].id);
      });
  }, []);
    ...
    if (isFormValid) {
      ...
      try {
        ...
        const response = await axios.post(
          'https://techstephub.focusrtech.com:3030/techstep/api/Timesheet/Service/update/wfhrequest',
          payload,
          {
            headers: {
              'Content-Type': 'application/json',
