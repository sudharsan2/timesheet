...
import { getAssetDetailsByIdAsync } from '../../redux/slices/assetSlice';
...
import { UserListHead, AssetListToolbar } from '../../components/_administrator/list';
...
export default function AssetDetails() {
  ...
  const { users: AssetDetails } = useSelector((state) => state.asset);
  ...
  useEffect(() => {
    dispatch(getAssetDetailsByIdAsync());
  }, [dispatch]);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = AssetDetails.map((n) => n.id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - AssetDetails.length) : 0;

  const filteredUsers = applySortFilter(AssetDetails, getComparator(order, orderBy), filterName);
