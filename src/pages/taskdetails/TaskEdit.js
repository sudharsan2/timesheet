  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        ...
        await dispatch(AssignedStatusChangeAsync(payload));
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            { name: 'Task Assigned', href: PATH_DASHBOARD.task.taskAssigned },
            { name: 'Create' }
          ]}
        />
                  ...
                  <Stack spacing={3}>
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                      ...
                      <FormControl fullWidth variant="outlined" size="large">
                        ...
                        >
                          {client.map((_x, i) => (
                            <MenuItem dense="true" key={i} value={_x.projectName} sx={{ height: '10px' }}>
                              {_x.projectName}
                            </MenuItem>
                          ))}
                        </Select>
