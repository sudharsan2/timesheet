...
import {
  getListOfMasterClassificationAsync,
  createOrUpdateMasterAsync,
  UpdateMasterAsync
...
export default function MasterEdit() {
  ...
  const masterDetails = masters.find((_x) => _x.master_id === Number(params.master_id));
  ...
  const formik = useFormik({
    ...
    initialValues: {
      master_id: masterDetails.master_id || '',
      classification: masterDetails.classification || '',
      country: masterDetails.country || '',
      state: masterDetails.state || '',
      city: masterDetails.city || '',
      company_name: masterDetails.company_name || '',
      business_vertical: masterDetails.business_vertical || '',
      contact_persion_name: masterDetails.contact_persion_name || '',
      email: masterDetails.email || '',
      alternative_email: masterDetails.alternative_email || '',
      country_code: masterDetails.country_code || '',
      phone_number: masterDetails.phone_number || '',
      // country_code_1: masterDetails.country_code_1 || '',
      alternative_phone_number: masterDetails.alternative_phone_number || ''
    },
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        ...
        await dispatch(UpdateMasterAsync(payload));
                  ...
                  <Stack spacing={3}>
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                      <TextField
                        ...
                        disabled
                      />
                      <FormControl fullWidth error={Boolean(errors.classification)}>
                        <InputLabel id="classification-id">Classification</InputLabel>
                        <Select
                          labelId="classification-id"
                          id="classification-id"
                          label="Classification"
                          {...getFieldProps('classification')}
                        >
                          {classifications.map((_x, i) => (
                      ...
                      <FormControl fullWidth error={Boolean(errors.country)}>
                        ...
                        <Select labelId="country-id" id="country-id" label="Country" {...getFieldProps('country')}>
                          {countryi.map((_x, i) => (
                            <MenuItem key={i} value={_x.label}>
                              {_x.label}
                            </MenuItem>
                          ))}
                        </Select>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                      <FormControl fullWidth error={Boolean(errors.country_code)}>
                        <InputLabel id="country_code-id">Country Code</InputLabel>
                        <Select
                          sx={{ width: 150 }}
                          labelId="country_code-id"
                          id="country_code-id"
                          label="Country code"
                          {...getFieldProps('country_code')}
                        >
                          {countryi.map((_x, i) => (
                            <MenuItem key={i} value={_x.phone}>
                              +{_x.phone}
                            </MenuItem>
                          ))}
                        </Select>
                      ...
                      />
                      {/* <FormControl fullWidth error={Boolean(errors.country_code_1)}>
                        <InputLabel id="country_code_1-id">Country Code</InputLabel>
                        <Select
                          sx={{ width: 150 }}
                          labelId="country_code_1-id"
                          id="country_code_1-id"
                          label="Country code"
                          {...getFieldProps('country_code_1')}
                        >
                          {countryi.map((_x, i) => (
                            <MenuItem key={i} value={_x.phone}>
                              +{_x.phone}
                        ...
                        <FormHelperText>{errors.country_code_1?.message}</FormHelperText>
                      </FormControl> */}
                      <TextField
                        sx={{ width: 1000 }}
