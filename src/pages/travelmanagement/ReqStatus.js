...
// redux
import { getListOfAllTravelDetailsAsync } from '../../redux/slices/projSlice';
...
import ReqStatusList from './ReqStatusList';
...
const TABLE_HEAD = [
  ...
  { id: 'hr_status', label: 'Status', alignRight: false, margin: '' },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user.hr_status.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function ProjectCreate() {
  ...
  const { statusreq: userList } = useSelector((state) => state.proj);
  ...
  useEffect(() => {
    dispatch(getListOfAllTravelDetailsAsync());
  }, [dispatch]);
  ...
  const title = 'Request Status List';
  return (
    <Page title={title}>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    const isItemSelected = selected.indexOf(empName) !== -1;
                      ...
                      >
                        ...
                        <TableCell>
                          <Button
                            ...
                            to={`${PATH_DASHBOARD.travel.approveStatus}/${travelId}`}
